...
public class MakeURI {
	...
	static String doc_uri(int line) {
		...
		for (int i = 0; i < Driver.classes[Driver.currentClass].methods.get(Driver.currentMethod).lineNumberTable.length; i++) {
			if (Driver.classes[Driver.currentClass].methods.get(Driver.currentMethod).lineNumberTable[i][1] <= line) {
				javaLine = Driver.classes[Driver.currentClass].methods.get(Driver.currentMethod).lineNumberTable[i][0] - 1;
				System.out
						.println("\t\t\t\t\t\t\t\t\t"
								+ (Driver.classes[Driver.currentClass].methods.get(Driver.currentMethod).lineNumberTable[i][0] - 1));
			}
		}

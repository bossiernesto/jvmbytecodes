...
elsif ($ARGV[$p] eq "In-order") {
    for ($i = 0; $i < (8 + int (rand(2))); $i++) {
	$data = $data . $numbers[$i] . " ";
    }
}
elsif ($ARGV[$p] eq "Reverse-order") {
    for ($i = (8 + int (rand(2))); 0 <= $i; $i--) {
	$data = $data . $numbers[$i] . " ";
    }
}
else {
...
# print ("java exe.insertionmyles.insertionSort " . $ARGV[0] . ".sho $data");
system ("java exe.insertionmyles.insertionSort " . $ARGV[0] . ".sho $data");

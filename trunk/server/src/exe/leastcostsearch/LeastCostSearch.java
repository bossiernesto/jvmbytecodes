...
/* file: LeastCostSearch.java
 * author: Andrew Jungwirth
 * date: 1 August 2005
 *
 * This class contains methods for generating GAIGS snapshots to demonstrate
 * the execution of the Least-Cost Search Algorithm. The main method receives 
 * the file name for the showfile via a command-line argument and then calls 
 * the necessary methods to write a series of snapshots to this file depicting 
 * a (more-or-less) randomly generated Least-Cost Search.
 */

package exe.leastcostsearch;
...
public class LeastCostSearch{
	...
	if(args.length > 1){
	    LeastCostSearch algo = new LeastCostSearch(args[0], true);
	}else{
	    LeastCostSearch algo = new LeastCostSearch(args[0], false);
	}
    }

    public LeastCostSearch(String filename, boolean new_data){
	...
	graph.setHeuristics(false);
	for(int i = 0; i < num_nodes; i++){
	    nodes[i].setHeuristic(0);
	}
	...
	int[] qWeights = { -1, -1, 0, 1, 1, 2, 2, 2, 3 };
	int qRef = 0, qLast = -1;
	...
	while(current.getChar() != nodes[goal].getChar()){
		...
		System.err.println("Least-Cost failed to find a path to the goal: " +
				   bigmiss.toString());
		out.close();
		System.exit(1);
	    }
	...
	out.println("<snap>");
	out.println("<title>Least-Cost Search Shortest Path from Start Node " +

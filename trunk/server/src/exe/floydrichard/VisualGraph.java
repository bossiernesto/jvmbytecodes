...
import java.util.regex.*; // added for gaigs regular expression
...
public class VisualGraph {
    ...
    private static final double EDGE_CENTER_MIN = 0.06;
    private static final double EDGE_LENGTH_MIN = 0.28;
    ...
    private int[][] distanceArray;	//for gaigs floyd algorythm
    private String[][] colorArray;	//for gaigs floyd algorythm
    private Vector[][] pathsArray;	//for gaigs floyd algorythm
    private boolean expert;			//for gaigs floyd algorythm
    ...
    public VisualGraph(String gaigsFile, boolean mode) {
        ...
        try {
            ...
            //fout.write(data);
            gaigsFloyd(out, 0); //goodStartNode);
            out.close();
        } catch(IOException e) {
            System.out.println("Error: " + e.toString());
        }
    ...
    public void randomGaigs(int numNodes, int numEdges) {
        ...
        initializeGraph(); // setup the graph
        clearGraph(); // setup the graph
        //numEdges += r.nextInt(2);
        ...
        while(left > 0) {
            ...
            my_edgeset[join[tmp2]][free[tmp]].setWeight(r.nextInt(5) + 1);
                        /*
                        if(r.nextInt(2) == 0)
                        {
                                my_edgeset[join[tmp2]][free[tmp]].activate();
                        ...
                        else
                        {
                                ...
                                my_edgeset[free[tmp]][join[tmp2]].setWeight(r.nextInt(5) + 1);
                        }
                         */
        ...
        while(i > 0) {
            x = r.nextInt(numNodes);
            y = r.nextInt(numNodes);
                        /*
                        if(x < y)
                        {
                                t = x;
                         ...
                         */
            if(x != y && ! my_edgeset[x][y].isActivated()
            && ! my_edgeset[y][x].isActivated()) {
                my_edgeset[x][y].activate();
                                /*
                                if(my_edgeset[y][x].isActivated())
                                        my_edgeset[x][y].setWeight(
                                                my_edgeset[y][x].getWeight());
                                else
                                 */
                my_edgeset[x][y].setWeight(r.nextInt(5) + 1);
                i--;
                num_edges++;
            }
        }
    ...
    public void drawDebugFile(String file) {
        try {
            ...
            for(int i = 0; i < num_nodes; i++) {
                out.print("Node:\t" + my_nodeset[i].getChar());
                out.print("\t" + my_nodeset[i].getX());
                out.println("\t" + my_nodeset[i].getY());
                out.println("Edge Array");
            }
            for(int x = 0; x < num_nodes; x++) {
                for(int y = 0; y < num_nodes; y++) {
                    if(my_edgeset[x][y].isActivated()) {
                        out.print((int) my_edgeset[x][y].getWeight() + " ");
                    } else {
                        out.print("X ");
                    }
                }
    ...
    public String gaigsSetListing(int[] set) {
        String retVal = "";
    ...
    /** prints a floyds gaigs animation to a given filestream */
    public void gaigsFloyd(PrintWriter out, int startNode) {
        int iterations = gaigsHowManyFloyd();
        Random r = new Random();
        questionCollection questions = new questionCollection(out, 5, iterations - 1);
        fibQuestion fibQ = null;
        fibQuestion fibHard = null;
        //mcQuestion visitOrderQ = null;
        int lastQ = -1;
        int count = 0;
        int[] qWeight = { 1, 1, 1, 0, 0 };
        if(expert) {
            qWeight[0] = 0;
            qWeight[1] = 0;
            qWeight[2] = 0;
        ...
        out.print("VIEW SCALE 0.80\n");
        out.print("VIEW DOCS novice1.html\n");
        Vector[][] nodesArray = new Vector[num_nodes][num_nodes];
        distanceArray = new int[num_nodes][num_nodes];
        colorArray = new String[num_nodes][num_nodes];
        pathsArray = new Vector[num_nodes][num_nodes];
                /*
                String[] colors = {"#F0FFFF", "#D7FFFF", "#F0FFFF", "#D7FFFF", "#F0FFFF", "#D7FFFF"};
                String startColor = "#33FFFF";  //cyan
                String endColor = "#FF33FF";	//magenta
                String mixColor = "#3333FF";	//blue
                String midColor = "#FFCC33";	//orange
                        else if(my_nodeset[n].getColor() == "pink")
                                out.print("\\#FF9933");
                        else if(my_nodeset[n].getColor() == "pink2")
                                out.print("\\#FFCC33");
                 */
        String[] colors = { "#CCFFFF", "#99FFFF", "#CCFFFF", "#99FFFF", "#CCFFFF", "#99FFFF" };
        String startColor = "#0066FF";  //blue
        String endColor = "#00FF00";	//green
        String mixColor = "#EFEF00";	//yellow
        String midColor = "#FFCC33";	//orange
        String newColor = "#FF9933";	//bright orange
        String endBuffer, midBuffer;
        
        // setup Floyds
        for(int xx = 0; xx < num_nodes; xx++) {
            for(int yy = 0; yy < num_nodes; yy++) {
                colorArray[xx][yy] = "#FFFFFF";
                pathsArray[xx][yy] = new Vector(5);
                nodesArray[xx][yy] = new Vector(5);
                if(xx == yy) {
                    distanceArray[xx][yy] = 0;
                } else if(my_edgeset[xx][yy].isActivated()) {
                    distanceArray[xx][yy] = (int) my_edgeset[xx][yy].getWeight();
                    pathsArray[xx][yy].addElement(my_edgeset[xx][yy]);
                    nodesArray[xx][yy].addElement(new Integer(xx));
                    nodesArray[xx][yy].addElement(new Integer(yy));
                } else {
                    distanceArray[xx][yy] = BIG_NUM;
                }
            }
        }
        
        drawGaigsNetwork(out, "Floyds Algorithm Initial Setup");
        out.print("VIEW DOCS novice2.php\n");
            ...
            for(int i = 0; i < num_nodes; i++) {
                for(int j = 0; j < num_nodes; j++) {
                    ...
                    if(h != i && h != j && i != j) {
                        ...
                        endBuffer = colorArray[h][j];
                        midBuffer = colorArray[i][j];
                        if(distanceArray[i][j]
                                > distanceArray[i][h] + distanceArray[h][j]) {
                            pathsArray[i][j].clear();
                            pathsArray[i][j].addAll(pathsArray[i][h]);
                            pathsArray[i][j].addAll(pathsArray[h][j]);
                            
                            nodesArray[i][j].clear();
                            nodesArray[i][j].addAll(nodesArray[i][h]);
                            nodesArray[i][j].addAll(nodesArray[h][j]);
                            ...
                            colorArray[i][j] = midColor;
                        }
                        colorArray[i][h] = startColor;
                        colorArray[h][j] = endColor;
                        
                        // color the edges for the first path
                        for(int iter = 0; iter < pathsArray[i][h].size(); iter++) {
                            ((VisEdge) pathsArray[i][h].get(iter)).setHexColor(startColor);
                        }
                        // color the edges for the second path
                        for(int iter = 0; iter < pathsArray[h][j].size(); iter++) {
                            if(((VisEdge) pathsArray[h][j].get(iter)).getHexColor() == startColor)
                                ((VisEdge) pathsArray[h][j].get(iter)).setHexColor(mixColor);
                            else
                                ((VisEdge) pathsArray[h][j].get(iter)).setHexColor(endColor);
                        }
                        
                        //color nodes if found a new path
                        if(newPath) {
                            for(int iter = 0; iter < nodesArray[i][h].size(); iter++) {
                                my_nodeset[((Integer) nodesArray[i][h].get(iter)).intValue()].setHexColor(
                                        startColor);
                            }
                            for(int iter = 0; iter < nodesArray[h][j].size(); iter++) {
                                my_nodeset[((Integer) nodesArray[h][j].get(iter)).intValue()].setHexColor(
                                        endColor);
                            }
                            my_nodeset[j].setHexColor(newColor);
                        }
                        
                        switch(lastQ) {
                            case 0:
                                fibQ.setAnswer("" + my_nodeset[i].getChar());
                                break;
                            case 1:
                                fibQ.setAnswer("" + my_nodeset[h].getChar());
                                break;
                            case 2:
                                fibQ.setAnswer("" + my_nodeset[j].getChar());
                                break;
                            case 3:
                                if(newPath && fibHard != null) {
                                    fibHard.setAnswer("" + distanceArray[i][j]);
                                    fibHard = null;
                                }
                                break;
                            case 4:
                                if(newPath && fibHard != null) {
                                    fibHard.setAnswer("" + my_nodeset[i].getChar()
                                    + my_nodeset[j].getChar());
                                    fibHard = null;
                                }
                                break;
                        }
                        ...
                        if(! expert || newPath)
                            lastQ = gaigsWeightedRandom(r, 5, qWeight);
                        
                        if(count < iterations - 1) {
                            switch(lastQ) {
                                case 0:
                                    fibQ = new fibQuestion(out, new Integer(++count).toString());
                                    fibQ.setQuestionText("The path that tries to reach the internal node starts at which node next?");
                                    questions.addQuestion(fibQ);
                                    questions.insertQuestion(fibQ.getID());
                                    break;
                                case 1:
                                    fibQ = new fibQuestion(out, new Integer(++count).toString());
                                    fibQ.setQuestionText("The next internal node is?");
                                    questions.addQuestion(fibQ);
                                    questions.insertQuestion(fibQ.getID());
                                    break;
                                case 2:
                                    fibQ = new fibQuestion(out, new Integer(++count).toString());
                                    fibQ.setQuestionText("The next destination node is?");
                                    questions.addQuestion(fibQ);
                                    questions.insertQuestion(fibQ.getID());
                                    break;
                                case 3:
                                    if(newPath) {
                                        fibHard = new fibQuestion(out, new Integer(++count).toString());
                                        fibHard.setQuestionText("What is the weight of the next shortest path found?");
                                        questions.addQuestion(fibHard);
                                        questions.insertQuestion(fibHard.getID());
                                    }
                                    break;
                                case 4:
                                    if(newPath) {
                                        fibHard = new fibQuestion(out, new Integer(++count).toString());
                                        fibHard.setQuestionText("Which nodes will the next shortest path join? (ie answer AB for a path from node A to node B)");
                                        questions.addQuestion(fibHard);
                                        questions.insertQuestion(fibHard.getID());
                                    }
                                    break;
                        ...
                        if(! expert || newPath) {
                            ...
                            message += my_nodeset[h].getChar()
                            + "  " + my_nodeset[i].getChar()
                            + "  " + my_nodeset[j].getChar()
                            + "   ( From " + my_nodeset[i].getChar()
                            + " to " + my_nodeset[j].getChar()
                            + " through " + my_nodeset[h].getChar()
                            + " )"; // Debug: " + iterations + " " + count;
                            
                            drawGaigsNetwork(out, message);
                        }
                        colorArray[i][j] = midBuffer;
                        colorArray[i][h] = colors[h];
                        colorArray[h][j] = endBuffer;
                        gaigsChangeHexEdgeColors(startColor, "#000000");
                        gaigsChangeHexEdgeColors(mixColor, "#000000");
                        gaigsChangeHexEdgeColors(endColor, "#000000");
                        
                        //color nodes back to white if found a new path
                        if(newPath) {
                            for(int iter = 0; iter < nodesArray[i][j].size(); iter++) {
                                my_nodeset[((Integer) nodesArray[i][j].get(iter)).intValue()].setHexColor(
                                        "#FFFFFF");
                            }
                        }
                        skip = 0;
    ...
    public void gaigsChangeHexEdgeColors(String search, String replace) {
        for(int i = 0; i < num_edges; i++) {
            for(int j = 0; j < num_edges; j++) {
                if(my_edgeset[i][j].getHexColor() == search) my_edgeset[i][j].setHexColor(replace);
            }
        }
    }
    ...
    // Counts the number of snapshots for the graph (result depends on expert setting)
    public int gaigsHowManyFloyd() {
        int count = 0;
        int[][] dist = new int[num_nodes][num_nodes];
        
        for(int i = 0; i < num_nodes; i++) {
            for(int j = 0; j < num_nodes; j++) {
                if(i == j) {
                    dist[i][j] = 0;
                } else if(my_edgeset[i][j].isActivated()) {
                    dist[i][j] = (int) my_edgeset[i][j].getWeight();
                } else {
                    dist[i][j] = BIG_NUM;
                }
            }
        ...
        for(int outer = 0; outer < num_nodes; outer++) {
            for(int mid = 0; mid < num_nodes; mid++) {
                for(int inner = 0; inner < num_nodes; inner++) {
                    if(outer != mid && outer != inner && mid != inner) {
                        if(! expert || dist[mid][inner] > dist[mid][outer] + dist[outer][inner]) {
                            count++;
                        }
                        dist[mid][inner] = Math.min(dist[mid][inner],
                                dist[mid][outer] + dist[outer][inner]);
                    }
                }
            }
        }
    ...
    // gaigsIsGoodFloyd
    // Written by Richard Teviotdale
    // Checks to make sure the graph has a good number of path solutions for floyds
    public boolean gaigsIsGoodFloyd() {
                /*
                // adaptation to force a pentagon
                ...
                for(int i = 0; i < num_nodes; i++)
                {
                        ...
                        for(int j = 0; j < num_nodes; j++)
                        {
                                if(i != j)
                                {
                                        if(my_edgeset[i][j].isActivated()) edgeCount++;
                                        if(my_edgeset[j][i].isActivated()) edgeCount++;
                                }
                        }
                        if(edgeCount != 3)
                        {
                                if(edgeCount == 5 && ! foundCenter)
                                {
                                        foundCenter = true;
                                }
                                else
    ...
    public void drawGaigsNetwork(PrintWriter out, String comment) {
        out.print("GraphArray\n1\n" + comment + "\n***\\***\n");
        
        //output the array dimensions
        out.println(num_nodes);
        out.println(num_nodes);
        
        //output the edge array
        for(int i = 0; i < num_nodes; i++) {
            for(int j = 0; j < num_nodes; j++) {
                out.println(my_nodeset[j].getChar());
                out.println(my_nodeset[i].getChar());
                out.print("\\" + colorArray[j][i]);
                if(distanceArray[j][i] == BIG_NUM)
                    out.println("Big");
                else
                    out.println(distanceArray[j][i]);
            }
        }
        out.println("***END_ARRAY***");
        
        //output the graph
        for(int n = 0; n < num_nodes; n++) {
            // print node number and coordinates
            out.print((n + 1) + " " +
                    ((my_nodeset[n].getX() - 0.5) * 0.5 + 0.80) + " " +
                    ((my_nodeset[n].getY() - 0.5) * 0.5 + 0.4) + "\n");
            // print the edges
            for(int e = 0; e < num_nodes; e++) {
                if(n != e && my_edgeset[n][e].isActivated()
                && ! Pattern.compile("#[Ff]{6}").matcher(
                        my_edgeset[n][e].getHexColor()
                        ).matches() ) {
                    out.print("\\A\\" + my_edgeset[n][e].getHexColor());
                    out.print((e + 1) + "\n" + (int)my_edgeset[n][e].getWeight() + "\n");
                }
            }
            out.print("32767\n");
            out.print("\\" + my_nodeset[n].getHexColor());
            // print node color / text
                        /*
                        if(my_nodeset[n].getColor() == "red")
                                ...
                                out.print("\\Y");
                        else if(my_nodeset[n].getColor() == "magenta")
                                out.print("\\M");
                        else if(my_nodeset[n].getColor() == "pink")
                                out.print("\\#FF9933");
                        else if(my_nodeset[n].getColor() == "pink2")
                                out.print("\\#FFCC33");
                        else if(my_nodeset[n].getColor() == "blue2")
                                out.print("\\L");
                         */
    ...
    public static void main(String[] args) {
        if(args[1].compareTo("expert") == 0) {
            VisualGraph myGraph = new VisualGraph(args[0], true);
        } else {
            VisualGraph myGraph = new VisualGraph(args[0], false);
        }
    }

...
public class VisualGraph {
    ...
    public VisualGraph(String gaigsFile) {
        ...
        for(int i = 0; i < 1000; i++) {
            randomGaigs(8 + tmp, 11 + tmp + r.nextInt(3));
    ...
    public void randomGaigs(int numNodes, int numEdges) {
        ...
        while(left > 0) {
            ...
            //my_edgeset[join[tmp2]][free[tmp]].setWeight(r.nextInt(4) + 1);
        ...
        while(i > 0) {
            ...
            if(!my_edgeset[x][y].isActivated()) {
                ...
                //my_edgeset[x][y].setWeight(r.nextInt(3) + 7);
    ...
    /** prints a gaigs animation to a given filestream */
    public void gaigs(PrintWriter out, int startNode) {
        ...
        int tf = 0;
        int qCount = 0;
        int[] prevNode = new int[num_nodes];
        String title = "Breadth first search stating from node " +
                my_nodeset[startNode].getChar();
// 		out.print("VIEW SCALE 0.75\n");
// 		out.print("VIEW DOCS index.html\n");
        
        // going to use a vector as a queue
        Vector nodeQueue = new Vector(num_nodes);
        
        // set up initial node data
        for(int i = 0; i < num_nodes; i++) {
            prevNode[i] = -1;
            my_nodeset[i].setClosed(false);
            my_nodeset[i].setText("open");
        }
        //my_nodeset[startNode].setColor("red");
        my_nodeset[startNode].setClosed(true);
        my_nodeset[startNode].setText("closed");
        nodeQueue.addElement(new Integer(startNode));
        
        // draw a snapshot
        drawGaigsNetwork(out, title + "\n" + "FRONT <-- " + gaigsQueueListing(nodeQueue));
        ...
        while(nodeQueue.size() > 0) {
            ...
            // pop the first thing off the queuea and set it's color blue
            int current = ((Integer) nodeQueue.elementAt(0)).intValue();
            nodeQueue.removeElementAt(0);
            //my_nodeset[current].setColor("blue");
            my_nodeset[current].setHexColor(new_closed);
            visitOrder += my_nodeset[current].getChar();
            
            // add to queue all the nodes that connect to this node by a single edge
            for(int i = 0; i < num_nodes; i++) {
                if(my_edgeset[current][i].isActivated() &&
                        !my_nodeset[i].isClosed()) {
                    //my_edgeset[i][current].setColor("pink");
                    //my_edgeset[current][i].setColor("pink");
                    my_edgeset[i][current].setHexColor(new_open_e);
                    my_edgeset[current][i].setHexColor(new_open_e);
                    //my_nodeset[i].setClosed(true);
                    //my_nodeset[i].setColor("pink");
                    my_nodeset[i].setHexColor(new_open_n);
                    nodeQueue.addElement(new Integer(i));
                    prevNode[i] = current;
                    thisStackAdd++;
                }
            }
            
            // and color the path back to the start node blue
            int bluePath = current;
            while(bluePath != startNode) {
                //my_edgeset[bluePath][prevNode[bluePath]].setColor("blue");
                //my_edgeset[prevNode[bluePath]][bluePath].setColor("blue");
                my_edgeset[bluePath][prevNode[bluePath]].setHexColor(new_closed);
                my_edgeset[prevNode[bluePath]][bluePath].setHexColor(new_closed);
                bluePath = prevNode[bluePath];
                thisDepth++;
            }
            
            // check if deepest path
            if(thisDepth > deepest) deepest = thisDepth;
            
            // answer the last question
            switch(lastQ) {
                case 0:
                    fibQ.setAnswer("" + my_nodeset[current].getChar());
                    break;
                case 1:
                    fibQ.setAnswer("" + thisDepth);
                    break;
                case 2:
                    fibQ.setAnswer("" + thisStackAdd);
                    break;
                default:
                    // do nothing (used to avoid answer upon loop entry)
            }
            
            // get a question ready
            if(++qCount > num_nodes - 1) {
                lastQ = -1;
            } else {
                if(qCount > 5 && specialQ > 1)
                    lastQ = 4;
                else if (qCount > 6 && specialQ > 0)
                    lastQ = 3;
                else
                    lastQ = r.nextInt(3 + specialQ);
                if(specialQ > 1 && lastQ == 3) lastQ = 4;
            }
            
            switch(lastQ) {
                case 0:
                    fibQ = new fibQuestion(out, new Integer(qCount).toString());
                    fibQ.setQuestionText("Which node will be visited the next time line 6 is executed?");
		    //                    fibQ.setQuestionText("Which node will be visited next?");
                    questions.addQuestion(fibQ);
                    questions.insertQuestion(fibQ.getID());
                    break;
                case 1:
                    fibQ = new fibQuestion(out, new Integer(qCount).toString());
                    fibQ.setQuestionText("When line 6 is next executed, how many edges deep will the path be to the next visited node?");
		    //                    fibQ.setQuestionText("How many edges deep will the next visited node be?");
                    questions.addQuestion(fibQ);
                    questions.insertQuestion(fibQ.getID());
                    break;
                case 2:
                    fibQ = new fibQuestion(out, new Integer(qCount).toString());
                    fibQ.setQuestionText("How many nodes will be added to the queue the next time a node is closed and the loop in lines 7-8 is executed?");
		    //                    fibQ.setQuestionText("How many nodes will be added to the queue next?");
                    questions.addQuestion(fibQ);
                    questions.insertQuestion(fibQ.getID());
                    break;
                case 3:
                    specialQ = 0;
                    tfQ = new tfQuestion(out, (new Integer(qCount)).toString());
                    questions.addQuestion(tfQ);
                    questions.insertQuestion(tfQ.getID());
                    break;
                case 4:
                    specialQ = 1;
                    mcQ = new mcQuestion(out, new Integer(qCount).toString());
                    mcQ.setQuestionText("In which order will the nodes be visited?");
                    questions.addQuestion(mcQ);
                    questions.insertQuestion(mcQ.getID());
                    break;
            }
            
	    drawGaigsNetwork(out, title + "\n" + "FRONT <-- " + gaigsQueueListing(nodeQueue));
            
            //my_nodeset[current].setColor("green");
            my_nodeset[current].setHexColor(old_closed_n);
            for(int i = 0; i < num_nodes; i++) {
                if(my_edgeset[current][i].isActivated() &&
                        !my_nodeset[i].isClosed()) {
                                        /*
                                        my_edgeset[i][current].setColor("black");
                                        my_edgeset[current][i].setColor("black");
                                        my_nodeset[i].setClosed(true);
                                        my_nodeset[i].setColor("pink2");
                                         */
                    my_edgeset[i][current].setHexColor(gray);
                    my_edgeset[current][i].setHexColor(gray);
                    my_nodeset[i].setClosed(true);
                    my_nodeset[i].setHexColor(old_open_n);
                }
            }
            
            // and color the path back to the start node back to black
            bluePath = current;
            while(bluePath != startNode) {
                //my_edgeset[bluePath][prevNode[bluePath]].setColor("black");
                //my_edgeset[prevNode[bluePath]][bluePath].setColor("black");
                my_edgeset[bluePath][prevNode[bluePath]].setHexColor(gray);
                my_edgeset[prevNode[bluePath]][bluePath].setHexColor(gray);
                bluePath = prevNode[bluePath];
            }
            
        }
        
        // set up the special multi choice question
        mcQ.addChoice(gaigsStringExplode(visitOrder));
        if(r.nextInt(2) == 0) {
            ...
            mcQ.addChoice(gaigsStringExplode(
        ...
        // set up the special true false question
        String tfText = "";
        int tfAnswer = 0;
        if(r.nextInt(2) == 0) {
            tfText = "The deepest path to any node that will be visited during the search is ";
	    //            tfText = "The deepest path to a visited node is ";
            tfAnswer = deepest;
        } else {
            tfText = "The depth of the final node that will be visited during the search is ";
	    //            tfText = "The depth of the last node visited is ";
            tfAnswer = thisDepth;
        }
        if(r.nextInt(2) == 0) {
            tfQ.setQuestionText(tfText + tfAnswer + " edges.");
            tfQ.setAnswer(true);
        } else {
            if(r.nextInt(2) == 0) {
                tfQ.setQuestionText(tfText + ++tfAnswer + " edges.");
    ...
    // pre conditions given a Vector containing only ints
    // post returns a string representation of the contents
    public String gaigsQueueListing(Vector v) {
        String retVal = "{";
        if(v.size() == 0) {
            retVal += " empty ";
        } else {
            retVal += " "  + my_nodeset[((Integer) v.elementAt(0)).intValue()].getChar();
            for(int i = 1; i < v.size(); i++) {
                retVal += ", " + my_nodeset[((Integer) v.elementAt(i)).intValue()].getChar();
            }
            retVal += ' ';
        }
            ...
            out.print(my_nodeset[n].getChar() + "\n");
                ...
                for(int n = 0; n < num_nodes; n++)
                {
                        ...
                        if(my_nodeset[n].getColor() == "red")
                                ...
                                out.print("\\#FFCC33");
                        else if(my_nodeset[n].getColor() == "blue")
                                out.print("\\B");
                        out.print(my_nodeset[n].getChar() + "\n"); //+
    ...
    public boolean gaigsIsOverlap() {
        ...
        //dont need to check this ( no weights )
                /*
    ...
    public void gaigsDepthFirst(PrintWriter out) {
    }
    /****************************************************

...
public class Exercise6 extends Exercise {
  ...
  protected int size;
  protected int [] casesAllowed;
  protected boolean correct;
  protected int caseIndex;
  protected String casesAllowedString;
  
  protected GAIGStext text;
  
  public Exercise6(ShowFile show, PolygonToBeClipped sp, ClippingPolygon cp, GAIGSprimitiveCollection pc,
    int edgeIndex, int size, int [] cases)
  {
    super(show, sp, cp, pc);
    ...
    for(int i = 0; i < size; ++i) {
      ...
      casesAllowedString += casesAllowed[i] + "";
    }
  }
  ...
  public void generateVisualization() throws IOException {
      ...
      for(int j = 0; j < input.size(); ++j) {
        ...
        if(edgeIndex == i){
          ...
          for(int k = 0; k < size; ++k) {
            if(casenum == casesAllowed[k]) {
              temp = true;
              break;
            }
          }
          correct = correct && temp;
          text = new GAIGStext(0.5, 0, ((correct || correct != oldcorrect) ? 
            "Case " + casenum + " occurred and only cases " + casesAllowedString + " may occur." : 
            "The polygons produced a case\n not allowed at a previous frame.") );
    ...
    if(correct) 
    {
      text = new GAIGStext(0.5, 0, "Well Done, for edge " + (edgeIndex+1) + " only\n cases " + casesAllowedString + " occurred.");
      show.writeSnap(Clip.TITLE, doc_uri(), make_uri(sp.getStartIndex(), sp.getEndIndex(),
        -1, stack, outvertices, (new int[]{0}), pseudo), pc, text);
    } else {
      text = new GAIGStext(0.5, 0, "For edge " + (edgeIndex+1) + ", a case\n other than " + casesAllowedString + " occurred.");
      show.writeSnap(Clip.TITLE, doc_uri(), make_uri(sp.getStartIndex(), sp.getEndIndex(),
        -1, stack, outvertices, (new int[]{0}), pseudo), pc, text);
    }

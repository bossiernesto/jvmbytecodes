...
class ProblemGenerator {
    ...
    private static final double NAMESPROB = .5;  //prob. that a name will be re-used in a different scope
	...
	if(data.getIsArray()){
		...
		if((evaluate == REF || evaluate == NAME || evaluate == MACRO) && loc.compareTo("a") == 0 && x == data.getIndex())
	    ...
	    //if evaluate is by ref, then value is a reference to 
	    //the variable in main else store the value itself
	    if((evaluate == REF || evaluate == NAME || evaluate == MACRO) && loc.compareTo("a") == 0)
		temp.add("*" + data.getPrevName());
	    else
		temp.add(String.valueOf(data.getValue()));
    ...
    public ArrayList getmainNames(){
    	return (ArrayList) mainNames;
    }//getmainNames
    
    public ArrayList getfuncNames(){
    	return (ArrayList) funcNames;
    }//getfuncNames
    
    public ArrayList getglobalNames(){
    	return (ArrayList) globalNames;
    }//getfuncNames

    public ArrayList getfuncArgNames(){
    	return (ArrayList) funcArgNames;
    }//getfuncNames
	...
	if (position == "global") {
	    ...
	    //makes sure that the variable name is not already
	    //used in the function or in the argument list
	    if((funcNames.contains(name)) || (funcArgNames.contains(name)))
	       temp = true;
	...
	int index = 0;
	String name = "x";  //the name value returned
	int counter = 0;  //prevents an infinate loop
	double random = Math.random();  //prob. that a var name will be reused
	double ranGlobal = Math.random(); //prob. that a random global name will be used
	double ranMain = Math.random(); //prob. that a random main name will be used
	boolean found = true;  //set to false if cannot use a var name again
	

	//if a name should be used again
	if(random <= NAMESPROB){
		if(position == "global" || position == "main")
		  found = false;   //cannot find a var to use again so set found to false
		else{
		  if(ranGlobal < NAMESPROB  && globalNames.size() > 0){
		     do {
	    		index = (int) (Math.random() * (globalNames.size()));
			name = (String)globalNames.get(index);
		     }while (containsName(position, name) && counter++ < 100);
		   }//if
		   
		   else if(ranGlobal >= NAMESPROB  && mainNames.size() > 0){
		     do {
	    		index = (int) (Math.random() * (mainNames.size()));
			name = (String)mainNames.get(index);
		     }while (containsName(position, name) && counter++ < 100);
		   }//else if
		   ...
		   if (counter >= 50)
		        found = false;	
		 }//else func_argument or function
		
	
	}//if 
	
	if (random > NAMESPROB || found == false){
	...
	//checks if variable = variable + variable
	    ...
	    if (temp2.getIsArray())  //check if array
		str = str + " = " + temp.getName() + "[" + temp.getIdxPos() + "]";

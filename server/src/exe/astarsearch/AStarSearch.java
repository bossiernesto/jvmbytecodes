 ...
 * the execution of the A* Alorithm. The main method receives the file name for
 * the showfile via a command-line argument and then calls the necessary 
 * methods to write a series of snapshots to this file depicting a 
 * (more-or-less) randomly generated A* Search.
 */

package exe.astarsearch;
...
public class AStarSearch{
    ...
    public static final String NEW_REOPENED = "#FF3232";
    public static final String OLD_REOPENED = "#FFA9A9";
    ...
    // This vector is needed to retrace paths in the search tree to nodes that
    // have been re-expanded.
    private Vector reopened;

    public static void main(String[] args){
	if(args.length > 1){
	    AStarSearch algo = new AStarSearch(args[0], true);
	}else{
	    AStarSearch algo = new AStarSearch(args[0], false);
	}
    }

    public AStarSearch(String filename, boolean new_data){
	...
	TreeNode currentT, successorT, reopenedT = null;
	int currentI;
	...
	int[] qWeights = { -1, -1, 0, 1, 2, 2, 3, 3, 4 };
	...
	out.println("<show>");
	out.println("<snap>");
	out.println("<title>A* Search Initial State with Start Node " +
		    nodes[start].getChar() + " and Goal Node " +
		    nodes[goal].getChar() + ":</title>");
	out.println("<doc_url>astarsearch.html</doc_url>");
	...
	while(current.getChar() != nodes[goal].getChar()){
		...
		for(int n = 0; n < num_nodes; n++){
			...
			if(!nodes[n].isClosed()){
			    ...
			    edges[old_pred][n].setHexColor("#999999");
			    edges[n][old_pred].setHexColor("#999999");
			    nodes[n].setClosed(false);
			    try{
				for(int i = 0; i < closed.size(); i++){
				    if(((VisNode) closed.get(i)).getChar() == 
				       nodes[n].getChar()){
					closed.removeElementAt(i);
					break;
				    }
				}
			    }
			    catch(ArrayIndexOutOfBoundsException how){
				System.err.println("Closed removal failed: " +
						   how.toString());
			    }

			    reopenedT = reopenNode(nodes[n]);
			    reopened.add(reopenedT);
			    ...
			    writeLists();
			    out.println("<pseudocode_url>index.php?line=7</pseudocode_url>");
			    writeStructs(true);
			}
...

		{
		    ...
		    if(reopenedT != null){
			reopenedT.setHexColor(OLD_REOPENED);
			reopenedT.getLineToParent().setHexColor(OLD_REOPENED);
			reopenedT = null;
			reopen = false;
		    }
		...
		case 3:
		    tfQ = 
			new XMLtfQuestion(out, new Integer(qRef++).toString());
		    tfQ.setQuestionText("In the next time through the while loop, a node will be taken off the closed list and will be added to the open queue with an updated cost.");
		    break;
		case 4:
		    mcQ = 
		...
		System.err.println("A* failed to find a path to the goal: " +
				   bigmiss.toString());
		out.close();
		System.exit(1);
	    }
	...
	out.println("<snap>");
	out.println("<title>A* Search Shortest Path from Start Node " +
		    nodes[start].getChar() + " to Goal Node " +
		    nodes[goal].getChar() + ":</title>");
	out.println("<pseudocode_url>index.php?line=12</pseudocode_url>");
	writeStructs(false);
	out.println("</snap>");
	...
	if(size == 0){
		...
		if(current.compareTo((VisNode) open.get(n)) < 0){
		    open.add(n, current);
		    break;
		}else if(n == (size - 1)){
		    open.add(current);
    ...
    private TreeNode reopenNode(VisNode reopen){
	String path = "";
	char pred = reopen.getPred();
	TreeNode parent, child;
	int index;

	path = pred + path;
	pred = nodes[graph.translateCharIndex(pred)].getPred();
	...
	child = parent.getChild();
	if(child != null){
	    while(child != null && child.getChar() != reopen.getChar()){
		child = child.getSibling();
	    }
	}
	...
	if(child == null){
		...
		parent = (TreeNode)reopened.get(i);
		index = path.indexOf(parent.getChar());

		if(index != -1){
		    for(int n = index + 1; n < path.length(); n++){
			parent = parent.getChild();
			while(parent.getChar() != path.charAt(n)){
			      parent = parent.getSibling();
			}
		    ...
		    child = parent.getChild();
		    if(child != null){
			while(child.getChar() != reopen.getChar()){
			    child = child.getSibling();
			}
		    }
		    if(child != null){
	...
	child.setHexColor(NEW_REOPENED);
	child.getLineToParent().setHexColor(NEW_REOPENED);
	...
	if(open.isEmpty()){
		...
		out.print(current.getChar() + "(" + current.getPred() + "-" +
			  current.getCost() + "+" + current.getHeuristic() + ") ");
	    }
	    if(open.size() > 10){
		out.println("... }");
	    }else{
	...
	if(show_key){
	    ...
	    out.println("<column>");
	    out.println("<list_item color = \"#00A1F4\">");
	    out.println("<label>New Open</label>");
	    out.println("</list_item>");
	    out.println("<list_item color = \"#88E6F4\">");
	    out.println("<label>Old Open</label>");
	    out.println("</list_item>");
	    out.println("</column>");
	    out.println("<column>");
	    out.println("<list_item color = \"#44FF44\">");
	    out.println("<label>New Closed</label>");
	    out.println("</list_item>");
	    out.println("<list_item color = \"#AAFFAA\">");
	    out.println("<label>Old Closed</label>");
	    out.println("</list_item>");
	    out.println("</column>");
	    out.println("<column>");
	    out.println("<list_item color = \"#FF3232\">");
	    out.println("<label>New Reopened</label>");
	    out.println("</list_item>");
	    out.println("<list_item color = \"#FFA9A9\">");
	    out.println("<label>Old Reopened</label>");
	    out.println("</list_item>");
	    out.println("</column>");
	    out.println("<column>");
	    out.println("<list_item color = \"#FFFF00\">");

...
public class VisualGraph {
    ...
    public VisualGraph(String gaigsFile) {
        ...
        try {
            ...
            //fout.write(data);
            gaigsKruskal(out, 0); //goodStartNode);
            out.close();
        } catch(IOException e) {
            System.out.println("Error: " + e.toString());
        }
    ...
    /** prints a kruskal gaigs animation to a given filestream */
    public void gaigsKruskal(PrintWriter out, int startNode) {
        //String edge_skipped = "#DF0022";
        String edge_skipped = "#D14F63";
        String edge_selected = "#00FF00";
        ...
        int largestSet = 1;
        Random r = new Random();
        String title = "Kruskal - ";
        int[] qWeight = { 6, 1, 2, 2 };
        
        Vector edgeList = new Vector(num_edges);
        Vector nodeV1 = new Vector(num_edges);
        Vector nodeV2 = new Vector(num_edges);
        int[] nodeList = new int[num_nodes];
        for(int i = 10; i < num_nodes + 10; i++) {
            nodeList[i - 10] = i;
        }
        title += gaigsSetListing(nodeList);
        
        //		out.print("VIEW SCALE 0.75\n");
        //out.print("VIEW DOCS index.html\n");
        drawGaigsNetwork(out, title);
        
        // go through the edges and place into edgeList by weight
        for(int i = 0; i < num_edges; i++) {
            for(int j = i; j < num_edges; j++) {
                if(my_edgeset[i][j].isActivated()) {
                    double weight = my_edgeset[i][j].getWeight();
                    int k = 0;
                    while(k < edgeList.size() &&
                            weight >= ((VisEdge) edgeList.elementAt(k)).getWeight()) {
                        k++;
                    }
                    edgeList.insertElementAt(my_edgeset[i][j], k);
                    nodeV1.insertElementAt(new Integer(i), k);
                    nodeV2.insertElementAt(new Integer(j), k);
                }
            }
        }
        
        // run throught the edges and choose ones that create union joins
        int color = 0;
        int currentEdge = 0;
        int lastQ = -1;
        
        for(int i = 1; i < num_nodes; i++) {
            int thisSkippedEdges = 0;
            
            int x = ((Integer) nodeV1.elementAt(currentEdge)).intValue();
            int y = ((Integer) nodeV2.elementAt(currentEdge)).intValue();
            
            while(nodeList[x] == nodeList[y]) {
                my_edgeset[x][y].setColor("red");
                my_edgeset[y][x].setColor("red");
                
                my_edgeset[x][y].setHexColor(edge_skipped);
                my_edgeset[y][x].setHexColor(edge_skipped);
                
                x = ((Integer) nodeV1.elementAt(++currentEdge)).intValue();
                y = ((Integer) nodeV2.elementAt(currentEdge)).intValue();
                thisSkippedEdges++;
            }
            
            my_edgeset[x][y].setColor("green");
            my_edgeset[y][x].setColor("green");
            
            my_edgeset[x][y].setHexColor(edge_selected);
            my_edgeset[y][x].setHexColor(edge_selected);
            
            if(nodeList[x] > 9 && nodeList[y] > 9) {
                nodeList[x] = color++;
            }
            
            int bigger = Math.max(nodeList[y], nodeList[x]);
            int smaller = Math.min(nodeList[y], nodeList[x]);
            
            int nodesInSet = 0;
            // update the nodeList
            for(int j = 0; j < num_nodes; j++) {
                if(nodeList[j] == bigger || nodeList[j] == smaller) {
                    nodesInSet++;
                    nodeList[j] = smaller; //nodeList[x];
                    switch(smaller) {
                        case 0:
                            my_nodeset[j].setColor("green");
                            break;
                        case 1:
                            my_nodeset[j].setColor("yellow");
                            break;
                        case 3:
                            my_nodeset[j].setColor("blue2");
                            break;
                        default:
                            my_nodeset[j].setColor("magenta");
                            
                    }
                    my_nodeset[j].setHexColor(old_closed_n);
                }
            }
            
            for(int j = 0; j < num_edges; j++) {
                for(int k = 0; k < num_edges; k++) {
                    if(j != k && my_edgeset[j][k].isActivated()
                    && my_edgeset[j][k].getHexColor() == new_open_e
                            && my_nodeset[j].getHexColor() == old_closed_n
                            && my_nodeset[k].getHexColor() == old_closed_n) {
                        my_edgeset[j][k].setHexColor(old_closed_e);
                    }
                }
            }
            
            my_nodeset[x].setHexColor(new_closed);
            my_nodeset[y].setHexColor(new_closed);
            
            if(nodesInSet > largestSet) largestSet = nodesInSet;
            
            title = "Kruskal - " + gaigsSetListing(nodeList);
            // first edge will always be added to tree so safe to init here
                        /*
                        if(i == num_nodes - 1)
                        {
                                title = "Kruskal - Last Edge (green)\n" + gaigsSetListing(nodeList);
                        }
                        else
                        {
                                title = "Kruskal - Good Edge (green)\n" + gaigsSetListing(nodeList);
                        }
                         */
            ...
            switch(lastQ) {
                case 0:
                    fibQ.setAnswer("" + my_nodeset[x].getChar() +
                            my_nodeset[y].getChar() + "\n" +
                            my_nodeset[y].getChar() +
                            my_nodeset[x].getChar());
                    qWeight[0]--;
                    break;
                case 1:
                    fibQ.setAnswer("" + thisSkippedEdges);
                    break;
                case 2:
                    fibQ.setAnswer("" + largestSet);
                    qWeight[2]--;
                    qWeight[1]++;
                    break;
                case 3:
                    fibQ.setAnswer("" + nodesInSet);
                    qWeight[3]--;
                    qWeight[1]++;
                    break;
                default:
            ...
            switch(lastQ) {
                case 0:
                    ...
                    fibQ.setQuestionText("Which edge will be selected next?");
                    questions.addQuestion(fibQ);
                    questions.insertQuestion(fibQ.getID());
                    break;
                case 1:
                    fibQ = new fibQuestion(out, new Integer(i).toString());
                    fibQ.setQuestionText("How many edges will be skipped over next?");
                    questions.addQuestion(fibQ);
                    questions.insertQuestion(fibQ.getID());
                    break;
                case 2:
                    fibQ = new fibQuestion(out, new Integer(i).toString());
                    fibQ.setQuestionText("In the next frame the largest set will contain how many nodes?");
                    questions.addQuestion(fibQ);
                    questions.insertQuestion(fibQ.getID());
                    break;
                case 3:
                    fibQ = new fibQuestion(out, new Integer(i).toString());
                    fibQ.setQuestionText("How many nodes will the set created by the next selected edge contain?");
                    questions.addQuestion(fibQ);
                    questions.insertQuestion(fibQ.getID());
                    break;
                                        /*
            ...
            // now fix up edge colors
            my_edgeset[x][y].setColor("blue");
            my_edgeset[y][x].setColor("blue");
            gaigsChangeEdgeColors("red", "black");
            gaigsChangeHexEdgeColors(edge_skipped, white); //gray);
            gaigsChangeHexEdgeColors(edge_selected, new_open_e);
            gaigsChangeHexEdgeColors(old_closed_e, new_open_e);
            gaigsChangeHexNodeColors(old_closed_n, old_open_n);
            gaigsChangeHexNodeColors(new_closed, old_open_n);
            currentEdge++;
        ...
        gaigsChangeHexNodeColors(old_open_n, old_closed_n);
        gaigsChangeHexEdgeColors(new_open_e, old_closed_e);
        drawGaigsNetwork(out, "Minimum Cost Spanning Tree");

...
public class Exercise7 extends Exercise {
  ...
  public Exercise7(ShowFile show, PolygonToBeClipped sp, ClippingPolygon cp, GAIGSprimitiveCollection pc,
  ...
  public void generateVisualization() throws IOException {
    ...
    if(input.size() != startSize) {
      text = new GAIGStext(0.5, 0, "The subject polygon does not have " + startSize + " vertices.");
      drawBad(output, tempLabels, -1, (new int[]{0}));
      return;
    }
    ...
    for(int i = 0; i < edges.size(); ++i) {
      ...
      changePseudoCodeWindow(0);
      text = new GAIGStext(0.5, 0, "Current number of vertices: " + input.size());
    ...
    if(input.size() == endSize) {
      text = new GAIGStext(0.5, 0, "Well Done, The final subject polygon\n has exactly " + endSize + " vertices.");
      show.writeSnap(Clip.TITLE, doc_uri(), make_uri(sp.getStartIndex(), sp.getEndIndex(),
        -1, stack, outvertices, (new int[]{0}), pseudo), pc, text);
    } else {
      text = new GAIGStext(0.5, 0, "The final subject polygon has exactly\n " + input.size() + " vertices but requires " +
        endSize + " vertices.");
      show.writeSnap(Clip.TITLE, doc_uri(), make_uri(sp.getStartIndex(), sp.getEndIndex(),
        -1, stack, outvertices, (new int[]{0}), pseudo), pc, text);
    }
  ...
  protected void drawBad(ArrayList<Point> output, ArrayList<String> tempLabels,
    int casenum, int [] lines) throws IOException
  {
    pc.clearPrimitives();
    pc.addCircle(0.049, 0.033, .015, "#999900", "#999900", "#999900", "s");

...
public class Exercise4 extends Exercise {
  ...
  public Exercise4(ShowFile show, PolygonToBeClipped sp, ClippingPolygon cp, GAIGSprimitiveCollection pc,
    int edgeIndex, int numTimesToOccur)
  {
    super(show, sp, cp, pc);
  ...
  public void generateVisualization() throws IOException {
    ...
    drawSpecial(output, tempLabels, -1, (new int[]{0}));
    ...
    {
      text = new GAIGStext(0.5, 0, "Well Done, all cases occurred " + numTimesToOccur + 
        " times for edge " + (edgeIndex+1));
      show.writeSnap(Clip.TITLE, doc_uri(), make_uri(sp.getStartIndex(), sp.getEndIndex(),
        -1, stack, outvertices, (new int[]{0}), pseudo), pc, text);
    } else {
      text = new GAIGStext(0.5, 0, "All cases did not occur " + numTimesToOccur + 
        " times for edge " + (edgeIndex+1));
      show.writeSnap(Clip.TITLE, doc_uri(), make_uri(sp.getStartIndex(), sp.getEndIndex(),
        -1, stack, outvertices, (new int[]{0}), pseudo), pc, text);
    }

// See "t.n. change" for changes I've made
...
/*               A Graph Generation Package
   ...
   Department of Computer Science and Information Systems
                      DePaul University
                     Chicago, IL  60604
          johnsonbaugh@depaul.edu, kalin@depaul.edu
...
of these properties:
     ...
     * simple or nonsimple
     ...
     We have tested this program under UNIX (HP workstation),
VAX-11 C, and Turbo C and found it to perform as described.
...
#include <time.h>
#include <stdlib.h>
/* stdlib.h, which may not be available on some antique systems,
   is needed to provide a #define for RAND_MAX, the largest
   int returned by rand.
*/

int  ran();           /* customized random number generator */
void print_graph();   /* to user-specified file */
void permute();       /* gives a random permutation */
void init_array();    /* initialize array a so that a[ i ] = i */
void swap();          /* swap two ints */
int get_int();        /* get user input & check for valid input */
/*** miscellany ***/
#define True         1
#define False        0
#define None         -999
...
void display_dimensions_menu( which )
   int which;
{
   // t.n. change
   // printf( "\n\t\t%s", dimension_prompts[ VertexIndex ] );
   menu -> parms.vertex_count = 6 + ran(4); //get_int( "\n\t\t" );
   if ( which == EdgesAndVertices ) {
      //printf( "\t\t%s", dimension_prompts[ EdgeIndex ] );
      menu -> parms.edge_count = menu -> parms.vertex_count + 3 + ran(5); //get_int( "\n\t\t" );
   }
}

void display_max_degree_menu()
{
   int  degree;
...
void display_max_weight_menu()
{
   // t.n. change
   // printf( "\n\t\t\tMaximum Edge Weight:  " );
   menu -> parms.max_weight = 25 + ran(30); // get_int
}

void display_outfile_menu( count, index1, index2 )
   int count, index1, index2;
{
   ...
   for ( i = 0; i < count; i++ ) {
      if ( i == 0 ) {
         ...
         strcpy(menu -> outfiles.outfile1, jb_file);
      }
      else if ( i == 1 ) {
...
int illegal_parms( files )
   int files;
{
   int  zero_vertex_or_edge_count(),
        bogus_file_name();
   void warning(), fix_imbalanced_graph();
 ...
 */
void isomorphic_graph_pair( vertices, spec_degree, first, second, third )
   int vertices, spec_degree;
...
/* This function serves the same purpose as strcmp.  However,
   the arguments in compare_strings are of type void *, thus
   ...
   (Notice that in ANSI C, the void * types are automatically converted to
   char * when strcmp is called.)
 */
int compare_strings( a, b )
   void *a, *b;
{
   return strcmp( a, b );
}

void rename_vertices( aliases, count )
   int *aliases, count;
{
...
void rename_edges( ptr, aliases, count )
   Record *ptr;
   int    *aliases, count;
{
 ...
 */
void random_connected_graph( v, e, max_wgt, weight_flag, out_file )
   int v, e, max_wgt, weight_flag;
   char *out_file;
{
 ...
 */
void random_graph( v, e, max_wgt, weight_flag, simple_flag, out_file )
   int v, e, max_wgt, weight_flag, simple_flag;
   char *out_file;
{
...
/* This function generates a random, not necessarily simple
   ...
   output file.
 */
void r_graph( v, e, max_wgt, weight_flag, out_file )
   int v, e, max_wgt, weight_flag;
   char *out_file;
{
   FILE *fp;
...
/* This function generates a random directed graph with
   ...
   of the call to fix_imbalanced_graph.)
 */
void random_digraph( v, e, max_wgt, weight_flag, simple_flag, out_file )
   int v, e, max_wgt, weight_flag, simple_flag;
   char *out_file;
{
   int i, j, count, index, *adj_matrix;
 ...
 */
void directed_acyclic_graph( v, e, max_wgt, weight_flag, out_file, dag_file )
   int v, e, max_wgt, weight_flag;
   char *out_file, *dag_file;
{
 ...
 */
void complete_graph( v, max_wgt, dir_flag, out_file )
   int  v, max_wgt, dir_flag;
   char *out_file;
{
   int i, j;
   FILE *fp;
...
/* This function writes a simple graph with a Hamiltonian cycle to one
   ...
   e <= v(v-1) if the graph is directed. (In this program,
   this assured because of the call to fix_imbalanced_graph.)
   ...
   of edges.
 */
void hamiltonian_cycle_graph( v, e, dir_flag, out_file, ham_file )
   int v, e, dir_flag;
   char *out_file, *ham_file;
{
 ...
 */
void network_flow_graph( vertices, edges, max_weight, outfile )
   int vertices, edges, max_weight;
   char *outfile;
{
   int  start = 0,
        finish = vertices - 1,
...
int unpicked_vertices_p( closed, n )
   int *closed, n;
{
   int  i = 1;
...
int ran( k )
   int k;
{
   static double x = RAND_MAX + 1.0;
   
   return ( ( int ) ( rand() * ( k / x ) ) );
}

void print_graph( v, e, out_file, adj_matrix, dir_flag )
   int v, e, dir_flag;
   char *out_file;
   int *adj_matrix;
{

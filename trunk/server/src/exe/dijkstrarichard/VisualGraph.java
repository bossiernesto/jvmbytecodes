...
public class VisualGraph {
    ...
    private static final double EDGE_CENTER_MIN = 0.08;
    private static final double EDGE_LENGTH_MIN = 0.22;
    ...
    public VisualGraph(String gaigsFile) {
        ...
        //randomConnectedGraph(5, 7, true, true, false, 1, 12);
        for(int i = 0; i < 1000; i++) {
            int tmp = r.nextInt(2);
            clearGraph();
            randomGaigs(8 + tmp, 10 + tmp + r.nextInt(2));
            goodStartNode = gaigsAcceptable(2);
            
            while(goodStartNode == -1) {
                clearGraph();
                randomGaigs(8 + tmp, 10 + tmp + r.nextInt(2));
                goodStartNode = gaigsAcceptable(2);
            }
    ...
    public void randomGaigs(int numNodes, int numEdges) {
        ...
        join = r.nextInt(numNodes);
        dropInt(free, join, numNodes);
        left = numNodes - 1;
        
        while(left > 0) {
            tmp = r.nextInt(left);
            my_edgeset[join][free[tmp]].activate();
            my_edgeset[join][free[tmp]].setWeight(r.nextInt(4) + 1);
            my_edgeset[free[tmp]][join].activate();
            my_edgeset[free[tmp]][join].setWeight(
                    my_edgeset[join][free[tmp]].getWeight());
            join = free[tmp];
    ...
    public void gaigs(PrintWriter out, int startNode) {
        ...
        int lastQ = -1;
        int[] qWeight = { 1, 1, 1, 3}; //, 1, 2, 2 };
        Random r = new Random();
        String visitOrder = ""; // + my_nodeset[startNode].getChar();
        
        //colors
        //String edge_blue = "#0000FF"; // dark blue
        //String node_blue = "#6666FF"; // light blue
        //String node_green = "#80B780"; // light green
        //String edge_green = "#40B740"; // dark green
        String edge_blue = "#00FF00"; // green
        String node_blue = "#00FF00"; // green
        String node_green = "#AAFFAA"; // light green
        String edge_green = "#208220"; // dark green
        String edge_updated_color = "#00A1F2";
        String node_faded_update_color = "#C6E6F4";
        String node_updated_color = "#61C3F4";
        String white = "#FFFFFF";
        String black = "#999999";
        ...
        String title = "Dijkstra's Algorithm Initial Setup - Start Vertex " +
        ...
        for(int i = 0; i < num_nodes; i++) {
            my_nodeset[i].setClosed(false);
            my_nodeset[i].setText("Big");
            my_nodeset[i].setCost(BIG_NUM);
            prev[i] = -1;
        }
        
        my_nodeset[startNode].setCost(0);
        my_nodeset[startNode].setText("0");
        
// 		out.print("VIEW SCALE 0.80\n");
// 		out.print("VIEW DOCS index.html\n");
        drawGaigsNetwork(out, title, prev);
        
        for(int i = 0; i < num_nodes; i++) {
            // get the next node to close
            int champ = BIG_NUM;
            for(int j = 0; j < num_nodes; j++) {
                if(!my_nodeset[j].isClosed() && my_nodeset[j].getCost() < champ) {
                    champ = my_nodeset[j].getCost();
                    close = j;
                }
            }
            visitOrder += my_nodeset[close].getChar();
            
            // close and color the next node
            my_nodeset[close].setClosed(true);
            my_nodeset[close].setHexColor(node_blue);
            
            // color the selected edge
            if(prev[close] != -1) {
                my_edgeset[close][prev[close]].setHexColor(edge_blue);
                my_edgeset[prev[close]][close].setHexColor(edge_blue);
            }
            
            int updatedCosts = 0;
            // figure out the nodes that have changing costs
            for(int j = 0; j < num_nodes; j++) {
                if(my_edgeset[close][j].isActivated()) {
                    int newCost = (int) my_edgeset[close][j].getWeight() +
                            my_nodeset[close].getCost();
                    if(my_nodeset[j].getCost() > newCost) {
                        my_nodeset[j].setCost(newCost);
                        my_nodeset[j].setText(String.valueOf(newCost));
                        my_nodeset[j].setHexColor(node_updated_color);
                        my_edgeset[j][close].setHexColor(edge_updated_color);
                        my_edgeset[close][j].setHexColor(edge_updated_color);
                        prev[j] = close;
                        updatedCosts++;
                    }
                }
            ...
            switch(lastQ) {
                case 0:
                    fibQ.setAnswer("" + my_nodeset[close].getChar());
                    break;
                case 1:
                    fibQ.setAnswer("" + updatedCosts);
                    break;
                case 2:
                    fibQ.setAnswer("" + my_nodeset[close].getCost());
                    break;
            }
            ...
            if(i > num_nodes - 2) {
                ...
                lastQ = gaigsWeightedRandom(r, 4, qWeight);
            }
            
            switch(lastQ) {
                case 0:
                    fibQ = new fibQuestion(out, new Integer(i).toString());
                    fibQ.setQuestionText("Which node will be closed next when lines 8-9 are executed?");
                    questions.addQuestion(fibQ);
                    questions.insertQuestion(fibQ.getID());
                    break;
                case 1:
                    fibQ = new fibQuestion(out, new Integer(i).toString());
                    fibQ.setQuestionText("How many nodes will have their cost updated next when lines 12-13 are executed within the loop at line 10?");
                    questions.addQuestion(fibQ);
                    questions.insertQuestion(fibQ.getID());
                    break;
                case 2:
                    fibQ = new fibQuestion(out, new Integer(i).toString());
                    fibQ.setQuestionText("What will be the final cost of the next node that's closed when lines 8-9 are executed?");
                    questions.addQuestion(fibQ);
                    questions.insertQuestion(fibQ.getID());
                    break;
                case 3:
                    visitOrderQ = new mcQuestion(out, new Integer(i).toString());
                    visitOrderQ.setQuestionText("What is the correct order in which the nodes will be closed before the algorithm terminates?");
            ...
            // get title ready and draw the current graph
            title = "";
            for(int openList = 0; openList < num_nodes; openList++) {
                //if(! my_nodeset[openList].isClosed())
                if(my_nodeset[openList].getHexColor() == node_faded_update_color
                        || my_nodeset[openList].getHexColor() == node_updated_color) {
                    title += my_nodeset[openList].getChar() + ", ";
                }
            ...
            if(prev[close] != -1) {
                my_edgeset[close][prev[close]].setHexColor(edge_green);
                my_edgeset[prev[close]][close].setHexColor(edge_green);
            }
            my_nodeset[close].setHexColor(node_green);
            gaigsChangeHexEdgeColors(edge_updated_color, black);
            gaigsChangeHexNodeColors(node_updated_color, node_faded_update_color);
        ...
        gaigsChangeHexEdgeColors(black, white);
        drawGaigsNetwork(out, "Minimum cost paths from " + my_nodeset[startNode].getChar(), prev);
    ...
    // gaigsChangeNodeColors
    // Richard Teviotdale
    // goes throught the node array and changes any node with the first specified
    // color into the second specified color
    public void gaigsChangeNodeColors(String search, String replace) {
        for(int i = 0; i < num_nodes; i++) {
            if(my_nodeset[i].getColor() == search) my_nodeset[i].setColor(replace);
        }
    }
    
    /** writes a gaigs snapshot to a PrintWriter filestream */
    public void drawGaigsNetwork(PrintWriter out, String comment, int[] prev) {
        //out.print("Network\n2\n" + comment + "\n***\\***\n");
        out.print("VIEW SCALE 0.80\n");
        out.print("VIEW DOCS index.html\n");
        ...
        for(int i = 0; i < num_nodes; i++) {
            ...
            // y axis label
            out.println("cost");
            out.print("\\" + my_nodeset[i].getHexColor());
            out.println(my_nodeset[i].getText());
        }
        ...
        for(int i = 0; i < num_nodes; i++) {
            ...
            if(prev[i] == -1) {
                ...
                out.println(my_nodeset[prev[i]].getChar());
            }
        }
        ...
        for(int n = 0; n < num_nodes; n++) {
            ...
            out.print((n + 1) + " " +
                    ((my_nodeset[n].getX() - 0.5) * 0.75 + 0.65) + " " +
                    ((my_nodeset[n].getY() - 0.5) * 0.65 + 0.4) + "\n");

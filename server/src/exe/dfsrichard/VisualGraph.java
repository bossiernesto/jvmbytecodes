...
public class VisualGraph {
    ...
    /** prints a depthfirst gaigs animation to a given filestream */
    public void gaigs(PrintWriter out, int startNode) {
        ...
        String title = "Depth first search starting at node " +
                my_nodeset[startNode].getChar();
        
        // going to use a vector as a stack
...
// 		out.print("VIEW DOCS index.html\n");
        drawGaigsNetwork(out, title + "\n" + "TOP <-- " + gaigsQueueListing(nodeQueue));
        ...
        while(nodeQueue.size() > 0) {
            ...
            // pop the first thing off the stack and set it's color blue
            ...
            // add to stack all the nodes that connect to this node by a single edge
            for(int i = num_nodes - 1; i > -1; i--) {
                if(my_edgeset[current][i].isActivated() &&
                    ...
                    nodeQueue.insertElementAt(new Integer(i), 0);
                    thisStackAdd++;
                }
            }
            ...
            switch(lastQ) {
                ...
                case 2:
                    fibQ = new fibQuestion(out, new Integer(qCount).toString());
                    fibQ.setQuestionText("How many nodes will be added to the stack the next time a node is closed and the loop in lines 7-8 is executed?");
                    questions.addQuestion(fibQ);
                    questions.insertQuestion(fibQ.getID());
                    break;
                case 3:
            ...
            // finish up by setting the colors back for the next iteration
            ...
            // color the path back to the start node back to black
            bluePath = current;
            while(bluePath != startNode) {
            ...
            //get any closed nodes off the front of the queue
            while (nodeQueue.size() > 0 ) {
                if(! my_nodeset[((Integer)nodeQueue.elementAt(0)).intValue()].isClosed()) {
                    break;
                }
                nodeQueue.removeElementAt(0);
            }

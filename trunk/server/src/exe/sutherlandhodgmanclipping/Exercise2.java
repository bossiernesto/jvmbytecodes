...
public class Exercise2 extends Exercise {
  ...
  public Exercise2(ShowFile show, PolygonToBeClipped sp, ClippingPolygon cp, GAIGSprimitiveCollection pc,
    int conditionNum, boolean created)
  {
    super(show, sp, cp, pc);
  ...
  public void generateVisualization() throws IOException {
    ...
    for(int i = 0; i < edges.size(); ++i) {
      ...
      for(int j = 0; j < input.size(); ++j) {
        ...
        draw(output, tempLabels, casenum, findDetermineLines(casenum)); 
        ...
        if(created) {
          conditionCounter += casenum == 2 || casenum == 4 ? 1 : 0;
        } else {
          conditionCounter += (!Edge.isPointInsideEdge(e, ref, s)) ? 1 : 0;
        }
        text = new GAIGStext(0.5, 0, conditionCounter + " vertices have been " + (created ? "created." : "eliminated."));
        drawResult(output, tempLabels, casenum, findExecuteLines(casenum));
    ...
    if(conditionCounter == conditionNum) {
      text = new GAIGStext(0.5, 0, "Well done, " + conditionCounter + " vertices were " + (created ? "created." : "eliminated."));
      show.writeSnap(Clip.TITLE, doc_uri(), make_uri(sp.getStartIndex(), sp.getEndIndex(),
        -1, stack, outvertices, (new int[]{0}), pseudo), pc, text);
    } else {
      text = new GAIGStext(0.5, 0, "Only " + conditionCounter + " vertices were " + 
        (created ? "created. " : "eliminated. ") + "Required " + conditionNum);
      show.writeSnap(Clip.TITLE, doc_uri(), make_uri(sp.getStartIndex(), sp.getEndIndex(),
        -1, stack, outvertices, (new int[]{0}), pseudo), pc, text);
    }

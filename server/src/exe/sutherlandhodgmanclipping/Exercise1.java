...
public class Exercise1 extends Exercise {
  ...
  protected int caseNumToOccur;
  protected int numTimesToOccur;
  protected int caseCounter;
  
  protected GAIGStext text;
  
  public Exercise1(ShowFile show, PolygonToBeClipped sp, ClippingPolygon cp, GAIGSprimitiveCollection pc,
    int caseNumToOccur, int numTimesToOccur)
  {
    super(show, sp, cp, pc);
    ...
    this.caseNumToOccur = caseNumToOccur;
  ...
  public void generateVisualization() throws IOException {
    ...
    for(int i = 0; i < edges.size(); ++i) {
      ...
      for(int j = 0; j < input.size(); ++j) {
        ...
        caseCounter += casenum == caseNumToOccur ? 1 : 0;
        
        changePseudoCodeWindow(1);
        text = new GAIGStext(0.5, 0, "Case " + caseNumToOccur + " occurred " + caseCounter + " times.");
    ...
    if(caseCounter == numTimesToOccur) {
      text = new GAIGStext(0.5, 0, "Well Done, Case " + caseNumToOccur + " occurred exactly "
      ...
      text = new GAIGStext(0.5, 0, "Case " + caseNumToOccur + " did not occur exactly " 
        + numTimesToOccur + " times.");
      show.writeSnap(Clip.TITLE, doc_uri(), make_uri(sp.getStartIndex(), sp.getEndIndex(),
        -1, stack, outvertices, (new int[]{0}), pseudo), pc, text);
    }
